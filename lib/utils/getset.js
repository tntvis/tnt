epeek.utils.api = function (who) {
    ...
    // Auxiliar function to get the name of a method
    var method_name = function (fun) {
	var fname = fun.toString();
	fname = fname.substr('function '.length);
	fname = fname.substr(0, ret.indexOf('('));
	console.log("METHOD NAME IS: " + fname);
	return fname;
    };
    ...
    var publish_method = function (method, opts) {
	...
	if (typeof (param) === 'object') {
	    mm.add_batch(param);
	    for (var p in param) {
		publish_method(p, opts)
	    }
	} else {
	    mm.add(param, opts.default_value);
	    publish_method(param, opts);
	}
    };
