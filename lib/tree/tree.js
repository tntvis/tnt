epeek.tree.tree = function (data) {
   ...
   // cluster is an optional parameter 
    ...
    eTree.find_node_by_field = function(mytree, field, value) {
	if (mytree === undefined) {
	    mytree = data;
	}
	if (mytree[field] === value) {
	    return mytree;
	}
	...
	for (var i=0; i<mytree.children.length; i++) {
	    var node = eTree.find_node_by_field(mytree.children[i], field, value);
	    if (node !== undefined) {
		return node;
	    }
	}
	...
	return eTree.find_node_by_field(mytree, "name", name);
    };
