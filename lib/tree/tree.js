...
// 	    }
    ...
    eTree.node_present = function (cbak) {
	...
	var new_children = [];
	for (var i=0; i<data.children.length; i++) {
	    var subtree = epeek.tree.tree(data.children[i]);
	    if (subtree.node_present(cbak)) {
		new_children.unshift(subtree.data());
	    } else {
		new_children.push(subtree.data());
	    }
	}
