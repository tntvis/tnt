...
// 	    }
	...
	var new_branchset = [];
	for (var i=0; i<data.branchset.length; i++) {
	    new_branchset.push(epeek.tree.tree(data.branchset[i]));
	}

	new_branchset.sort(cbak);
	data.branchset = [];
	for (var i=0; i<new_branchset.length; i++) {
	    data.branchset.push(new_branchset[i].data());
	}
