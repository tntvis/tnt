epeek.tree = function () {
 "use strict";
    ...
    // Default colors
    var linkColor = 'steelblue';
    var nodeColor = 'steelblue';

    // Default node circle size
    var nodeCircleSize = '4.5';
	...
	var max_leaf_label_length = function (tree) {
		...
		var label_width = label.width()(leaves[i].data());
		if (label_width > max) {
		    max = label_width;
		}
	    }
	...
	var max_leaf_label_length = max_leaf_label_length(curr.tree);
	layout.max_leaf_label_width(max_leaf_label_length);
...
// 	cluster.size([n_leaves * label.height()(),
// 		      (layout.width() - layout.max_leaf_label_width() - layout.translate_vis[0]) - 15]);

	layout.adjust_cluster_size(cluster_size_params);
...
//	    .attr("height", (n_leaves * label.height()()) + 20)
	    .attr("height", layout.height(cluster_size_params) + 30)
	    .attr("fill", "none");
	...
	new_node.on("click", tree.node_info_callback);
	new_node.on("dblclick", tree.node_dbl_info_callback);
...
// 			  (layout.width() - layout.max_leaf_label_width() - layout.translate_vis[0]) - 15]);
		...
		.duration(duration)
		.ease(ease)
		.attr("height", layout.height(cluster_size_params) + 30); // height is in the layout
//		.attr("height", (n_leaves * label.height()()) + 20);
...
// 	    }
    ...
    //  -- Scale is now a property of the layout
...
//	prev.tree = base.tree;
	...
	tree.prune_by_names = function(node_names) {
		var curr_nodes = [];
		var orig_tree = tree.tree();
		var orig_data = tree.data();
		for (var i = 0; i < node_names.length; i++) {
		...
		if(curr_nodes && curr_nodes.length >1){
			tree.subtree(curr_nodes);
		}
	}

	tree.toggle_node = function (node_data) {
		tree.tree().toggle_node(node_data);
		return tree;
	}
	tree.focus_node = function (node_data) {
		// find 
		var found_node = tree.tree().find_node_by_field('_id', node_data._id);
...
//     };
    ...
    tree.node_info_callback = function () {};
	tree.node_dbl_info_callback = function () {};

    tree.tooltip = function (tooltip) {
    ...
    tree.link_color = function (color) {
	if (!arguments.length) {
	    return linkColor
	}
	linkColor = color;
    ...
    tree.node_color = function (color) {
	if (!arguments.length) {
	    return nodeColor;
	}
	nodeColor = color;
    ...
    tree.node_circle_size = function (size) {
	if (!arguments.length) {
	    return nodeCircleSize;
	}
	nodeCircleSize = size;
	return tree;
    }
