...
//    var stree = base.tree;
	...
	tree.update = function(is_new) {
		...
		// so this may be broken for 2+ transitions
		...
		console.log("EXITING ELEMENT IS:");
		console.log(this);
		for (var i=0; i<ls.length; i++) {
		    ...
		    var new_d = diagonal.call(this,{source : d.source, target : ls[i].target});
		    ...
		    // 	.transition()
		    // 	.duration(2000)
		    // 	.delay((max_depth_exit_node - depth) * 2000)
		    // 	.ease("lineal")
		    // 	.attr("d", diagonal.call(this,{source : d.source, target : ls[i].target}));
		    // 	// .attr("d", move_path(link.attr("d"), parent_link.attr("d")));
		}
	    };

	    // var move_path = function (path, parent_path) {
	    // 	var parts = path.split(" ");
	    // 	var parts1 = parts.shift();
	    // 	// var m_orig = part1.match("^M(.*),(.*)");

	    // 	var parent_parts =  parent_path.split(" ");
	    // 	var parent_parts1 = parent_parts.shift();
	    // 	// var m_parent = parent_parts1.match("^M(.*),(.*)");

	    // 	var new_parts = parts.unshift(parent_parts1);
	    ...
	    // 	// The regexp may be better defined outside (toplevel?)
	    // 	return function (d,i,a) {
	    // 	    var parts = a.split(" ");
	    // 	    var part1 = a.shift();
	    // 	    var orig = part1.match("^M(.*),(.*)");
	    // 	    return function(t) {
...
// 	    };
	    ...
	    // We stagger pending transitions
	    link
	    	.each(function(d) {
		    ...
		    pending_transitions.sort(function(a,b) {return b.delay<a.delay})
			...
			l
			    ...
			    .duration(2000)
			    .ease("lineal")
			    .delay(pending_transitions[i].delay*2000)
			    .attr("d",pending_transitions[i].path);
		    }
		    // console.log("////");
		    // console.log(pending_transitions);
