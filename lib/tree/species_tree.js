...
//    var base_tree;
	...
	tree.update = function(is_new) {
	    ...
	    // For a given moving link the parent is pulled to keep integrity
	    var pull_parent = function (d) {
		console.log("PULL_PARENT");
		// "this" is the moving link
		// So, take the parent link in the prev tree and pull it back
		...
		// Looking for the first node that is not in the current tree,
		// but was in the previous one
		// so skipping if this node doesn't have a parent or it has but is in the subTree
		if ((orig_source_node.parent === undefined) || (orig_source_node.parent.__inSubTree__.prev)) {
		    return;
	        }
		...
		// At this point orig_source_node is a node that has a parent that is not
		// currently in the subtree
		// ie. it is a target node of a new link
		var node_id = orig_source_node.__epeek_id__;
		// if (parent_id === undefined) {
		//     console.log("PARENT_ID IS UNDEF");
		//     return
		// }
		...
		console.log("LOOKING FOR A LINK WITH TARGET __epeek_id__ of " + node_id)
		var l = find_link_by_field(base_links,
					   "target",
					   "__epeek_id__",
					   node_id);
		console.log("FOUND L IS:");
		console.log(l);
		var link = d3.select("#ePeek_tree_link_" + div_id + "_" + l.target.__epeek_id__);
		var link_node = link.node();

		link
		    .transition()
		    .duration(duration)
		    .delay((max_depth_exit_node + 2) * duration)
		    .attr("d", diagonal.call(link_node, {
			source : d.source,
			target : l.target
		    }));
...
// 		    path  : new_d,
// 		    delay : ((max_depth_exit_node+1) * duration)
// 		});

	    };

	    var create_new_link = function (d) {
		// this is the new link DOM element
		// d is the link node
	    ...
	    // TODO: If new is true is it better to avoid transition?
	    // or maybe transition very fast?
	    // First, create the link with source and target pointing to the same position
	    // Then, move the path to the final position
	    // Entering links
	...
	newtree.apply(newtree.data(), function(d) {d.__inSubTree__ = {prev : true}});
	...
	base.tree.apply(base.data, function(d){d.__inSubTree__.curr = undefined});
//	base.tree.apply(base.data, function(d){d.__inSubTree__ = undefined});
