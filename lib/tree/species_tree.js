// Based on the code by Ken-ichi Ueda in http://bl.ocks.org/kueda/1036776#d3.phylogram.js
  epeek.species_tree = function (newick) {
    ...
    var sp_counts = {};
      ...
      // 	  switch(tree.layout()) {
      // 	  case "radial" :
      //         vis.attr("transform", "translate(" + r + "," + (r * 1.3) + ")");
	...
	var layouts = {
	    'radial' : {
		'translate_vis'  : function() {vis.transition().duration(2000).attr("transform", "translate(" + r + "," + (r*1.3) + ")")},
	    ...
	    'vertical' : {
		'translate_vis'  : function() {vis.transition().duration(2000).attr("transform", "translate(20,20)")},
	...
	vis = d3.select(div)
	    .append("svg")
	    .attr("width", width)
	// TODO: This has probably to be adjusted for the vertical layout
	    .attr("height", width * 1.3 )
	    .attr("fill", "none")
	    .append("g");

	tree.transition = function() {
	    var cluster = layouts[layout].cluster;
	    var diagonal = layouts[layout].diagonal();
	    var transform = layouts[layout].transform_node;
	    layouts[layout].translate_vis();


	    // We get a new tree here using the default newick
	    // What is returned are the nodes of the tree
	    // TODO: Have an API to set the newick
	    epeek_tree = epeek.tree(newick, cluster);
	    species_tree = epeek_tree.tree();

	    console.log("EIN?");
	    console.log(vis.selectAll("path.ePeek_tree_link"));
	    if (vis.selectAll("path.ePeek_tree_link")[0].length ) {
		console.log("--- BEFORE:");
		console.log(vis.selectAll("path.ePeek_tree_link").attr("d"))
	    }
	...
	// tree.update(sp_counts);
	  ...
	  var d = function (diagonalPath, i) {
	      ...
	      console.log("PATHDATA FOR: " + source.name + "-" + target.name);
    ...
    // 	if (n.length != null) {
    // 	    offset += n.length * 50; // This number should be adjusted "manually". Gives the size of the tree
    // 	}
