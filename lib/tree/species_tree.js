...
// 		}
		...
		for (var i=0; i<ls.length; i++) {
		    var link_node = d3.select("#ePeek_tree_link_" + div_id + "_" + ls[i].target.__epeek_id__).node();
...
//			.attrTween("transform", translateAlong(d3.select(this).node()))
			.attr("d", diagonal.call(link_node, {source : ls[i].source.parent, target : ls[i].target.parent}));
		}
