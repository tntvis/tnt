...
//    var base_tree;
	...
	tree.update = function(is_new) {
		...
		return get_direct_parent(node.parent);
	    };

	    // For a given moving link the parent is pulled to keep integrity
	    var pull_parent = function (d) {
		// "this" is the moving link
		// So, take the parent link in the prev.data and pull it back
		var prev_tree = prev.tree;
		var prev_data = prev.data;
		console.log("PREV_DATA:");
		console.log(prev_data);
		console.log("__epeek_id__" + d.source.__epeek_id__);
		var prev_source_node = prev_tree.find_node_by_field(prev_data,
								    "__epeek_id__",
								    d.source.__epeek_id__);
		console.log("PREV_SOURCE_NODE");
		console.log(prev_source_node);
		var parent_id = get_direct_parent(prev_source_node);
		...
		for (var i=0; i<links.length; i++) {
		    var link_data = d3.select(links[i]).datum();
		    if (d.target.__epeek_id__ === link_data.source.__epeek_id__) {
		...
		for (var i=0; i<ls.length; i++) {
		    ...
		    var new_d = diagonal.call(link_node,{source : d.source, target : ls[i].target});
		    link_node.__pending_transitions__.push({path  : new_d,
	    ...
	    // We schedule pending transitions
	    link
	    	.each(function(d) {
