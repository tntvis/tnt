...
//     var epeek_tree;
	...
	// We need to know if this is a true update or the first time the tree is plotted.
	// We use is_new for that
	tree.update = function(is_new) {
		...
		if (prev_link === undefined) {
		    prev_link = find_link_by_field(prev.links,
						   "target",
						   "__epeek_id__",
						   orig_target_node.__epeek_id__);
		} else {
		    d3.select(this)
		    // This is now needed because the exiting links were not removed and are located in the previous pos
