...
//     var epeek_tree;
	...
	var link = vis.selectAll("path.ePeek_tree_link")
	    .data(curr.links, function(d){return d.target.__epeek_id__});
	link
	    	.enter()
		.append("path")
	...
	var node = vis.selectAll("g.ePeek_tree_node")
	    .data(curr.nodes, function(d) {return d.__epeek_id__});
	...
	var new_node = node
	    ...
	    .attr("id", function(d) {
		return "ePeek_tree_node_" + div_id + "_" + d.__epeek_id__
	    })
	...
	tree.update = function() {
	    ...
	    // If prev tree doesn't exist, it is created based on the current one (if any):
	    if ( (prev === undefined) && (curr.links !== undefined) && (curr.nodes !== undefined) ) {
		prev.nodes = [];
		copy_nodes(curr.nodes, prev.nodes);

		prev.links = [];
		copy_links(curr.links, prev.links);
	    }

	    // Set up the current and prev tree
	    // The prev tree has to be set up here because the cluster may have changed
	    // var nodes = curr.tree.cluster(cluster).nodes();
	    // var links = cluster.links(nodes);
	    curr.nodes = curr.tree.cluster(cluster).nodes();
	    curr.links = cluster.links(curr.nodes);

	    prev.nodes = prev.tree.cluster(cluster).nodes();
	    prev.links = cluster.links(prev.nodes);

	    // TODO: This method is always called with the __epeek_id__ of the target node
	    // maybe it is better to rewrite it
		...
		var curr_source_node = nodes[0];
		var curr_target_node = nodes[1];

		if ((curr_source_node.parent === undefined) ||
		    (curr_source_node.__inSubTree__.prev) ||
		    (!curr_target_node.__inSubTree__.prev)) {
		    return false;
		}
		return true;
	    }
		...
		for (var i=0; i<node.branchset.length; i++) {
		    ids.push(node.branchset[i]);
		    get_all_downstream_links(node.branchset[i], ids);
		}
	    };

	    // Move 'this' if its parent link is missing
	    // 'd' refers to the moving link in the current tree
	    var pull_parent = function(d, pull_delay, prev_link) {
		...
		// This gets the same link we are moving in the prev tree
		if (prev_link === undefined) {
		    prev_link = find_link_by_field(prev.links,
						   "target",
						   "__epeek_id__",
						   d.target.__epeek_id__);
		} else {
			...
			.attr("d", diagonal.call(this, {
			    source : prev_link.source,
			    target : prev_link.source
			}))
		}

		// Moving 'this' link to its final position but still linking with where it was
		d3.select(this)
		    .transition()
		    .duration(duration)
		    .ease("lineal")
		...
		// Also the corresponding node is moved
		d3.select("#ePeek_tree_node_" + div_id + "_" + d3.select(this).datum().target.__epeek_id__)
		    .transition()
		    .duration(duration)
		    .ease("lineal")
		    .delay((max_depth_exit_node + pull_delay) * duration)
		...
		var downstream_links = []
		get_all_downstream_links(orig_target_node, downstream_links);
		for (var i=0; i<downstream_links.length; i++) {
		    var successor = find_first_successor(downstream_links[i], function (d) {return d.__inSubTree__.prev});
		    if (successor !== undefined) {
			var link = d3.select("#ePeek_tree_link_" + div_id + "_" + downstream_links[i].__epeek_id__);
			link
			    .transition()
			    .duration(duration)
			    .ease("lineal")
...
// 			var node = d3.select("#ePeek_tree_node_" + div_id + "_" + downstream_links[i].__epeek_id__);
...
// 			    .attr("transform", transform);
		...
		// We repeat the proces with the parent only if it is also 'entering'
		if ((orig_source_node.parent !== undefined) &&
		    (orig_source_node.parent.__inSubTree__.curr) &&
		    (!orig_source_node.parent.__inSubTree__.prev)){
		    ...
		    pull_parent.call(parent_link_node, parent_link, (pull_delay+1), prev_link);
		}
		
	    };

	    // This method is run on prev nodes
	    // It has to traverse the tree returning the first
	    // node in the current tree on each downstream branch
	    var get_all_downstream_nodes = function (node, children) {
		if (node.branchset === undefined) {
		    return;
		}
		for (var i=0; i<node.branchset.length; i++) {
		    if (node.branchset[i].__inSubTree__.curr) {
			children.push(node.branchset[i].__epeek_id__);
		    } else {
			get_all_downstream_nodes(node.branchset[i], children);
		    }
		}
	    };


	    // For a given exiting link the downstream links are pulled to keep integrity
	    var pull_children = function (d) {
		var depth = d.target.depth;

		var prev_tree = prev.tree;
		var prev_data = prev_tree.data();
		var prev_target_node = prev_tree.find_node_by_field(prev_data,
								    '__epeek_id__',
								    d.target.__epeek_id__);

		// children_ids has the ids of the first child on each downstream branch
		// i.e. the ids of the children that need to be pulled by exiting links / nodes
		var children_ids = [];
		get_all_downstream_nodes(prev_target_node, children_ids)

		var ls = [];
		var prev_nodes = prev.nodes;
		var prev_links = prev.links;

		for (var i=0; i<children_ids.length; i++) {
		    var l = find_link_by_field(prev_links,
					       "target",
					       "__epeek_id__",
					       children_ids[i])
		    ls.push(l);
	    ...
	    var find_first_ancestor = function (node, callback) {
		if (callback(node)) {
//		if (node.__inSubTree__.prev) {
		...
		return find_first_ancestor(node.parent, callback);
	    };

	    var find_first_successor = function (node, callback) {
		if (callback(node)) {
//		if (node.__inSubTree__.prev) {
		    return node;
		}
		...
		for (var i=0; i<node.branchset.length; i++) {
		    return find_first_successor(node.branchset[i], callback);
		}
	    };

	    var get_new_link_delay = function (d) {
		var base_delay = (max_depth_exit_node + entering_links);
		var link_length = this.getTotalLength();

		if (this.getTotalLength() === 0) {
		    var curr_target = d.source;
		    while (true) {
			// Get the parent link
			var link_node = d3.select("#ePeek_tree_link_" + div_id + "_" + curr_target.__epeek_id__).node();
			if (link_node === null) {
			    break;
			}
			if (link_node.getTotalLength() === 0) {
			    base_delay++;
			    curr_target = d.source.parent;
			    continue;
			}
			break;
		    }
		...
		return base_delay * duration;
	    };

	    var get_new_link_path = function (d) {
		// d is the new link to be created
		// To position it, we look for its first ancestor in the prev tree
		// For that, the orig tree is traversed up
		var orig_tree = tree.tree();
		var orig_data = orig_tree.data();
		var orig_source_node = orig_tree.find_node_by_field(orig_data,
								    "__epeek_id__",
		...
		var orig_target_node = orig_tree.find_node_by_field(orig_data,
								    "__epeek_id__",
								    d.target.__epeek_id__);
		// If the new node doesn't have any successor in the prev tree, it is not
		// placed in the first ancestor, but in its final position with length 0
		if (find_first_successor(orig_target_node, function (d) {return d.__inSubTree__.prev}) === undefined) {
		    return diagonal.call(this, {
			source : d.source,
			target : d.source
		    });
		}

		var first_prev_ancestor = find_first_ancestor(orig_source_node, function(d) {return d.__inSubTree__.prev});
		return diagonal.call(this, {
		    source : first_prev_ancestor,
		    target : first_prev_ancestor
		});
	    };
...
//	    	.delay((max_depth_exit_node + entering_links) * duration) // TODO: I changed this (from 1). Not sure it is correct
	...
	for (var i=0; i<curr_nodes.length; i++) {
	    orig_tree.upstream(curr_nodes[i], function(d) {d.__inSubTree__.curr = true});
	}
