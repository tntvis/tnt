...
//     var epeek_tree;
	...
	tree.update = function(is_new) {
	    ...
	    if (is_new){ 
		vis.attr("transform", "translate("+layouts[layout].translate_vis[0]+","+layouts[layout].translate_vis[1]+")");
	    } else {
		vis.transition().duration(2000).attr("transform", "translate("+layouts[layout].translate_vis[0]+","+layouts[layout].translate_vis[1]+")");
	    }

	    // We set up the tree
	    var nodes = stree.cluster(cluster).nodes();
	    // epeek_tree = epeek.tree(newick, cluster);
	    // species_tree = epeek_tree.data();

	    var link = vis.selectAll("path.ePeek_tree_link")
		.data(cluster.links(nodes), function(d){return d.target.__epeek_id__});
	    ...
	    var exit_link = link
		.exit();
		// .each(function(d) {d.source.delay_factor = curr_delay_factor}) // Mark the nodes for removal
		//     .each(function(d) {
                //         check_upstream(d.source);
                //     }
		// 	 )

	    var descendants = exit_link
		.selectAll("ePeek_tree_link");
		...
		.each(move_descendents)
		.attr("stroke-dashoffset", 0)
		.transition()
		.ease("lineal")
		// .delay(function(d) {return d.source.delay_factor * 2000})
		.delay(function(d) {return (max_depth_exit_node - d.target.depth) * 300})
		.duration(300)
	...
	copy_data (orig_data.branchset[0], sub_data);
	stree = epeek.tree(sub_data);
