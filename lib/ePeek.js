...
//            .transition().duration(dur).attr("fill", fgColor);
        ...
        names.on("click", function(d) {gBrowser.gene_info_callback(d)});
...
///*********************////
	...
	Once the orthologues are retrieved, it the {@link ePeek.orthologues_callback} callback is invoked passing the array of orthologues as its argument.
...
///***********************////
	...
	If no argument is provided, it returns the current species.
	...
	If no argument is provided, it returns the current chr or the default one if no one has been set before.
	...
	If no argument is provided, it returns the current start coordinate or the default one if none has been set before.
	This value is used by {@link ePeek.start} to set the genomic coordinates in the plug-in view
	@param {Number} [coordinte] The new start coordinate
	@returns {ePeek} The original object allowing method chaining
     */
	...
	If no argument is provided, it returns the current start coordinate or the default one if none has been set before.
	This value is used by {@link ePeek.start} to set the genomic coordinates in the plug-in view
	@param {Number} [coordinate] The new start coordinate
	@returns {ePeek} The original object allowing method chaining
     */
...
///*********************////
    ...
    /** <strong>gene_info_callback</strong> is a callback that tells the plug-in how
	to show any info associated with a gene.
	Its default behaviour is to do nothing.
        This function can be overwritten by a theme to display the gene information
	in a custom way and place
	It expects a literal object as its unique argument having the following fields:
	<ul>
    ...
    */
    ...
    /** <strong>ensGenes_callback</strong>
     */
    gBrowser.ensGenes_callback = function() {};

    /** <strong>orthologues_callback</strong>
     */
