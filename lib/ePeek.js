var epeek = function() {
    ...
    */
	...
	var span = currDomain[1] - currDomain[0];
	var extraspan = (span * factor) - span;
	xScale.domain([currDomain[0] - ~~extraspan/2, currDomain[1] + ~~extraspan/2]);
	zoom();
    };

    // TODO: To make this move smooth, one idea is to update the positions of all the elements
    // but this seems to complicated, we have to think about a better option
	...
	var span = currDomain[1] - currDomain[0];
	var move = (span * factor) - span;
	console.log("MOVE:" + move);
	xScale.domain([~~(currDomain[0] + move), ~~(currDomain[1] + move)]);
	zoom();
    };
