...
var zoom = function (new_xScale) {
    ...
    var isZoom
    if (d3.event && d3.event.sourceEvent.type === "mousewheel") {
	isZoom = true;
    } else {
	isZoom = false;
    }
    ...
    var newCapLeft = currDomain[0];
    var newCapRight = currDomain[1];

    var onLim = false;

    if (currDomain[0] <= limits.left) {
	onLim = true;
    ...
    var finalLeft = d3.max([limits.left, newCapLeft]);
    var finalRight = d3.min([limits.right, newCapRight]);
    xScale.domain([finalLeft, finalRight]);
    if (isZoom && drag_allowed) {
