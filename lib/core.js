...
gBrowser.start = function () {
    ...
    eRest.call({url : eRest.url("chr_info",
		...
		success : function (resp) {
		    ...
		    limits.right = d3.min([chr_length, limits.right]);
...
gBrowser.allow_drag = function(b) {
	...
	if (xScale !== undefined) {
	    pane.call( zoomEventHandler.x(xScale).xExtent([0, limits.right]);scaleExtent([(toPos-fromPos)/(limits.zoomOut-1), (toPos-fromPos)/limits.zoomIn]).on("zoom", zoom) );
