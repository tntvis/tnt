var epeek = function() {
    ...
    */
    ...
    // 	    species    : species,
    // 	    chr        : chr,
    // 	    on_success : function(resp) {chr_length = resp.length}
    // 	});
    ...
    gBrowser.start = function () {
	eRest.call(eRest.url("chr_info",
			     {species : species,
			      chr : chr
			     }),
		   function (resp) {chr_length = resp.length});

	eRest.call(eRest.url("region",
			     {species : species,
			      chr     : chr,
			      fromPos : fromPos,
			      toPos   : toPos
	...
	if (currDomain[1] > chr_length) {
	    ...
	    eRest.call(eRest.url("region", {species : species,
					    chr     : chr,
					    fromPos : fromPos,
					    toPos   : toPos
					   }),
    ...
    // };
    gBrowser.get_gene = function (gene_name) {
	eRest.call(eRest.url('species_gene', { species : species,
					       gene_name : gene_name 
					     }
			    ),
		   function(resp) {
...
///*********************////
    ...
    gBrowser.homologues = function(ensGene) {
	eRest.call(eRest.url("homologues", {gene_name : ensGene}),
		   function(resp) {
    ...
    gBrowser.ensGene_lookup = function (gene_name) {
	gBrowser.homologues(gene_name);
	eRest.call(eRest.url("gene", {gene_name : gene_name}),
		   function(resp) {
		       gBrowser
			   .species(resp.species)
			   .chr(resp.seq_region_name)
