...
gBrowser.start = function () {
    ...
    eRest.call({url : eRest.url("chr_info",
		...
		success : function (resp) {stop_activity(); chr_length = resp.length; limits.right = d3.min([chr_length, limits.right]);}
	       }
	      );
...
var plot = function () {
    ...
    // zoom
    if (drag_allowed) {
        pane.call( zoomEventHandler.x(xScale).scaleExtent([(toPos-fromPos)/(limits.zoomOut-1), (toPos-fromPos)/limits.zoomIn]).on("zoom", zoom) );
    }
};
...
// Going back to old buggy zooming-when-panning-at-the-edge behavior.
// For now I'm going to wait until this pull request (https://github.com/mbostock/d3/pull/1299) gets its way into d3
// Then we will have xExtent for free and ready to use
// Or just use the fork or apply the PR to a local d3 copy
...
var zoom = function (new_xScale) {
    ...
    currDomain[0] = Math.max(currDomain[0], limits.left);
    currDomain[1] = Math.min(currDomain[1], limits.right);
...
/** <strong>limits</strong> sets the left, right, zoom-in and zoom-out limits of the plug-in in. Units in nucleotides. Left and right should denote the leftmost and rightmost positions allowed. Zoom-in and zoom-out denotes the maximum span in nucleotides. The hard limits for left and right are 0 and the chromosome (seq_region) lenght respectively. These limits can not be passed. The hard limits for zoom-in and zoom-out are 200 and the limit imposed by the rest service (if any). If no argument is provided returns the current limits.
    @param {Object} Object An object containing any combination of the fields 'left', 'right', 'zoomIn' and 'zoomOut'
    @returns {ePeek} The original object allowing method chaining
*/
gBrowser.limits = function (lims) {
    if (!arguments.length) {
	return limits;
    }
    if ((lims.left !== undefined) && (lims.left > 0)) { // hard left-limit is 0
	limits.left = lims.left;
    }
    if (lims.right !== undefined) { // We need to check that this is not going past the chr length (but not now, because we don't know it)
	limits.right = lims.right;
    }
    if ((lims.zoomOut !== undefined) && (lims.zoomOut < eRest.limits.region)) { // hard zoom-out limit is imposed by the rest api
	limits.zoomOut = lims.zoomOut;
    }
    if (lims.zoomIn !== undefined) { // Any value is acceptable
	limits.zoomIn = lims.zoomIn;
    }
    return gBrowser;
};
...
gBrowser.allow_drag = function(b) {
	...
	if (xScale !== undefined) {
	    pane.call( zoomEventHandler.x(xScale).scaleExtent([(toPos-fromPos)/(limits.zoomOut-1), (toPos-fromPos)/limits.zoomIn]).on("zoom", zoom) );
//	    zoomEventHandler.x(xScale).scaleExtent([(toPos-fromPos)/(limits.zoomOut-1), (toPos-fromPos)/limits.zoomIn]).on("zoom", zoom);
	}
    } else {
