var genomeBrowser = function () {
    ...
    var gene_info_div_id  = undefined;

    // Handler for genes and gene names
    var gene_selected_cbak = function(){};
...
///*********************////
    // Private functions
    var highlight = function (elem, gene, highlight) {
	var classname = "ePeek_" + d3.select(elem).attr("data-geneName");
	if (highlight) {
	    console.log("FIRED1");
//	    d3.selectAll("." + classname).transition().duration(durFill).attr("fill", "orange"); // Should this color be optional??
	    d3.select("#" + gene_info_div_id + " p").remove();

	    d3.select("#" + gene_info_div_id)
//	    d3.select("#ePeek_" + div_id + "_gene_info")
		...
		.style("background-color", bgColor)
		    ...
		    return "<h1>" + gene.external_name + "</h1>" +
                        ...
                        "Type of gene: <i>" + gene.feature_type + "</i><br />" +
                        "loc: <i>" + gene.seq_region_name + ":" + gene.start + "-" + gene.end + " (Strand: " + gene.strand + ")</i><br />"
                })

	    // This is needed for ePeek_mobile.html
	    gene_selected_cbak(gene);

	    return true;
	} else {
	    d3.selectAll("." + classname).transition().duration(durFill).attr("fill", fgColor);
    ...
    var update = function () {
       svg_g.call(xAxis);
        ...
        rects.enter()
	    // .append("a")
	    // .attr("class", "flip") // Needed for mobile app
	    // .attr("href", "#gene_info")
            .append("rect")
            .attr("class", function(d) {return "ePeek_gene ePeek_" + d.ID})
	    .attr("data-geneName", function(d) {return d.ID})
            .attr("x", function (d) {
                return (xScale(d.start))
	    })
            .attr("y", 10)
        ...
        rects.on("click", function(d) {highlight(this, d, true)});
        ...
        names.on("click", function(d) {highlight(this, d, true)});
...
///*********************///
	...
	if (gene_info_div_id === undefined) {
	    gBrowser.gene_info_div_id("ePeek_" + div_id + "_gene_info");
	}
    };
    ...
    // gene_selected_callback sets the passed callback to be called when a gene is selected
    // (clicked on the gene or its name)
    // It allows for custom actions based on the click event and the gene selected.
    // The gene is passed as its argument
    gBrowser.gene_selected_callback = function(cbak) {
	if (!arguments.length) {
	    return gene_selected_cbak;
	}
	gene_selected_cbak = cbak;
	return gBrowser;
    };
...
//	gene    = undefined;
    ...
    // gene_info_div_id sets a different html element to display the info of a selected gene
    // This is used ofr example in ePeek_mobile.html
    // The expected argument in the id of the html element
    gBrowser.gene_info_div_id = function(id) {
	if (!arguments.length) {
	    return gene_info_div_id;
	}
	gene_info_div_id = id;
	return gBrowser;
    };
    ...
    // gB.species("human").chr(13).from(35,009,587).to(35,214,822).gene("LINC00457");
