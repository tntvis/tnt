...
var genomeBrowser = function () {
	...
	d3.jsonp(url, function (resp) {
	    console.log("RESP: ");
	    console.log(resp);
	    var sel = d3.select("#" + divElem)
	    .append("select")
	    .attr("id", function(){return divElem + "_gene_select"});
    ...
    gBrowser.div = function (d) {
		...
		.attr("value", "Jump!")
		.on("click", function(d){
		    d3.select("#" + divElem + "_gene_select").remove();
		    gBrowser.gene(document.getElementById(divElem + "_gene_name").value);
		});
            p
		.append("text")
		.text("(example: ENSG00000139618 or SNORD73)");
        }
