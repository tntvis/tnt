...
/// RENDERING FUNCTIONS
    ...
    gBrowser.update = function () {
       svg_g.call(xAxis);
        ...
        rects.on("mouseover", function(d) {highlight(this, d, true)});
        ...
        names.on("mouseover", function(d) {highlight(this, d, true)});
...
/// DATA RETRIEVERS
	...
	d3.json(url, function (error, resp) {
	    ...
	    d3.select("#" + n_orth_div_id)
	    	.text(resp.data[0].homologies.length);
	});
    };
...
//	gene    = undefined;
	...
	if (n_ensGenes_div_id === undefined) {
	    gBrowser.n_ensGenes_div_id("ePeek_" + div_id + "_n_ensGenes");
	}
	if (n_orth_div_id === undefined) {
	    gBrowser.n_orthologues_div_id("ePeek_" + div_id + "_n_orthologues");
	}
    };
    ...
    gBrowser.n_ensGenes_div_id = function(id) {
	if (!arguments.length) {
    ...
    gBrowser.n_orthologues_div_id = function(id) {
	if (!arguments.length) {
