...
// Returned closure
	...
	if (searchBox) {
		...
		.on("click", function(d) {
			...
			gBrowser.ensGene(document.getElementById(div_id + "_gene_name").value, ensGene);
		    } else {
...
/// DATA RETRIEVERS
	...
	console.log("MY URL:");
	    ...
	    species = resp.species;
            resp.start = resp.start; //32889611;
            resp.end = resp.end; // 32973805;
            resp.chr = resp.chr; //13;
...
/// GETTERS & SETTERS:
    ...
    gBrowser.orthologues = function (b) {
	show_orthologs = b;
	return gBrowser;
    }; 
