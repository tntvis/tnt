...
// Returned closure
	...
	// The Options div
	var opts = d3.select(div)
	    .append("div")
	    .attr("class", "ePeek_toplevel")
	    .attr("id", "ePeek_options_pane");
	...
	// QRtag div
	var qrtagLabel = d3.select(div)
	    .append("div")
	    .attr("class", "ePeek_qrtag_label") // both needed?
	    .attr("id", "ePeek_qrtag_label")
	    .text("QR code")
	    .style("display", function() {if(show_qr_code){return "block"} else {return "none"}})
	...
	if (gene !== undefined) {
//	    gBrowser.get_gene(gene, ensGeneBox);
...
/// GETTERS & SETTERS:
    ...
    gBrowser.show_qr_code = function(b) {
	show_qr_code = b;
	return gBrowser;
    };
...
/// UTILITY FUNCTIONS:
    gBrowser.buildLink = function() {
	var url = "http://www.ebi.ac.uk/~mp/clients/client_mobile.html?loc="
	    + species + ":" + chr + ":" + fromPos + "-" + toPos;
	return url;
    };
