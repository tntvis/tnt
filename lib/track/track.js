...
epeek.track.track = function() {
    ...
    // TODO: Not all the tracks will need this. Make this optional or even better create one on demand for each
    // track that needs it.
    ...
    // plotter is the code used to plot the track
    var plotter;
    ...
    newtrack.plotter = function (callback) {
	if (!arguments.length) {
	    return plotter;
	}
	plotter = callback;
...
epeek.track.track.gene = function () {
...
epeek.track.track.block = function() {
    var track = epeek.track.track()
	.display("block")
	.index("start");
...
epeek.track.track.pin = function() {
...
epeek.track.track.empty = function () {

    var track = epeek.track.track();
    var updater = track.retriever.local();
    track.update(updater);

    return track;
};

epeek.track.track.location = epeek.track.track.empty
epeek.track.track.axis = epeek.track.track.empty;
