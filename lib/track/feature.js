...
//    var layout = epeek.track.layout.identity(); // For the genes track this has to be set to epeek.track.layout.gene()
    ...
    // Getters / Setters
    epeek.utils.getset(feature)
	.getset('create', create_cbak)
	.getset('mover', move_cbak)
	.getset('updater', update_cbak)
	.getset('info', info_cbak)
	.getset('layout', epeek.track.layout.identity());
	...
	new_elem.on("click", feature.info());
	// new_elem is a g element where the feature is inserted
	feature.create().call(track, new_elem, xScale);
    };

    feature.update = function (xScale) {
	var track = this;
	var svg_g = track.g;
	var layout = feature.layout();
	...
	feature.updater().call(track, vis_elems);
	...
	feature.mover().call(this, elems, xScale);
    };
