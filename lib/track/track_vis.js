...
epeek.track = function() {
    ...
    var api_right = function (factor) {
	if (factor > 0) {
	    _manual_move(factor, 1);
	}
    };

    var api_left = function (factor) {
	if (factor > 0) {
	    _manual_move(factor, -1);
	}
    };

    var api_zoom = function (factor) {
	_manual_move(factor, 0);
    };

    var api_reorder = function (ids) {
	var new_tracks = [];
    ...
    var api_add_track = function (track) {
	tracks.push(track);
	return track_vis;
    };

    // 
    var api_width = function (w) {
