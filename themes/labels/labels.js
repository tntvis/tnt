var epeek_theme = function() {
	...
	// Different labels
	var no_label = epeek.tree.label.text()
	    .text(function (d) {
		return "";
	    })
	    .width(function (d) {
	...
	var as_is_label = epeek.tree.label.text(); // Default options (ie. unchanged names)
	var clean_label = epeek.tree.label.text() // Same as default but without underscores
	    .text(function (d) {
		return d.name.replace(/_/g, ' ');
	    });
	var prefix_label = epeek.tree.label.text() // Showind only 7 characters
	    .text(function (d) {
		return d.name.substr(1,7) + "...";
	    })
	    .width(function (d) {
		return 10;
	    });
	...
	sT
	    ...
	    .duration(2000)
	    .layout(epeek.tree.layout.radial().width(600).scale(false))
	    .label(clean_label);
