var epeek_theme_tree_compare = function () {
    ...
    var label_en = epeek.tree.label.text()
            .text(function (node) {
                if (node.children) {
                    return "";
                } else {
	...
	var label_nhx = epeek.tree.label.text()
            .text(function (node) {
                if (node.children) {
                    return "";
                } else {
    ...
    var sel = d3.select(div)
		...
		if (this.value === 'Example Tree') {
		    ...
		    .width(430)
		    .scale(false))
	    	.label (epeek.tree.label.text().height(height));
		}

		if (this.value === 'Example Tree 2') {
		    tree
	    	.data (epeek.tree.parse_newick (newick2))
	    	.layout (epeek.tree.layout.vertical()
		    .width(430)
		    .scale(false))
	    	.label (epeek.tree.label.text().height(height));
